...
import {
    AlertTriangle, CheckCircle2, Clock, Package, Timer, TrendingDown, TrendingUp, Users,
} from 'lucide-react'
...
function MetricCard({ label, value, helper, isLoading, icon: Icon, iconColor = 'hsl(var(--primary))', trend }: MetricCardProps) {
    const trendClass =
        ...
        <Card>
            <CardHeader className="pb-2">
                ...
                {isLoading
                    ...
                    : <CardTitle className="text-2xl font-bold text-[hsl(var(--foreground))]">{value}</CardTitle>
                }
            </CardHeader>
                ...
                {isLoading
                    ...
                    : (
                        <div className="flex items-center gap-1.5">
                            {trend && (
                                <span className={cn('flex items-center gap-0.5 text-[11px] font-semibold', trendClass)}>
...
export default function AnalyticsPage() {
    ...
    const trendsParams = useMemo(
        () => ({ from: appliedRange.from, to: appliedRange.to, granularity: 'day' as const }),
        [appliedRange],
    )

    const { data: summaryData, isLoading: summaryLoading }           = useAnalyticsSummary({ from: appliedRange.from, to: appliedRange.to })
    const { data: trendsData,  isLoading: trendsLoading }            = useAnalyticsTrends(trendsParams)
    const { data: districtVolumeData, isLoading: districtVolumeLoading } = useDistrictVolume({ from: appliedRange.from, to: appliedRange.to })
    const { data: demandForecastData, isLoading: demandForecastLoading } = useDistrictDemandForecast({ hoursAhead: forecastHoursAhead })
    const { data: driverAnalyticsData }                               = useDriverAnalytics()
    const { data: sarimaForecast }                                    = useSarimaDeliveryForecast(3)
    const { data: statusBreakdownData, isLoading: statusBreakdownLoading } = useStatusBreakdown({ from: appliedRange.from, to: appliedRange.to })
    ...
    return (
        <div className="flex flex-col gap-6 p-6">
            <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <h1 className="text-2xl font-semibold tracking-tight text-[hsl(var(--foreground))]">Analytics</h1>
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <div className="flex items-center gap-2">
                        <Button variant="outline" size="sm" onClick={() => downloadCsv('range')}>Download CSV</Button>
                        <Button variant="ghost"   size="sm" onClick={() => downloadCsv('full')}>Full CSV</Button>
                    </div>
                </div>
            </header>
            ...
            <div className="flex flex-col gap-6">
                ...
                <section className="grid gap-4 lg:grid-cols-2">
                    <Card>
                        ...
                        <CardContent>
                            <DeliveryTrendChart data={deliveryTrend} forecast={sarimaForecast} isLoading={trendsLoading} />
                        </CardContent>
                    </Card>
                ...
                <section className="grid gap-4 lg:grid-cols-2">
                    <Card>
                        ...
                        <CardContent>
                            <DistrictVolumeChart
                                data={(districtVolumeData?.items ?? []).map((item) => ({
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <StatusBreakdownChart data={statusBreakdownData?.items ?? []} isLoading={statusBreakdownLoading} />
                        </CardContent>
                    </Card>
                </section>
                ...
                <section>
                    <Card>
                        <CardHeader>
                            <div className="flex items-start justify-between gap-2">
                                <div>
                                    ...
                                    <CardDescription className="text-xs text-[hsl(var(--foreground-muted))]">
                                        Seasonal-naive forecast (28-day same hour/day-of-week average), summed over the selected lookahead window.
                                    </CardDescription>
                                </div>
                                <div className="flex shrink-0 items-center gap-1.5 rounded-lg border border-[hsl(var(--primary)/0.4)] bg-[hsl(var(--primary)/0.08)] px-2.5 py-1.5">
                        ...
                        <CardContent>
                            <DistrictDemandForecastChart data={top5DemandForecast} isLoading={demandForecastLoading} />
                        </CardContent>
                    </Card>
                </section>

                {/* Urgency + Drivers at risk */}
                <section className="grid gap-4 lg:grid-cols-2">
                    <Card>
                        <CardHeader>
                            <CardTitle className="text-sm font-semibold uppercase tracking-widest text-[hsl(var(--foreground-muted))]">
                        ...
                        <CardContent>
                            <UrgencyTrendChart data={urgencyTrend} isLoading={trendsLoading} />
                        </CardContent>
                    </Card>
                                    ...
                                    {driversAtRisk.map((d) => (
                                        <div key={d.driverId} className="flex items-center gap-3 py-2.5">
                                            ...
                                            <div className="flex items-center gap-3 text-xs tabular-nums shrink-0">
                                                <span className={d.anomalyRate >= 0.15 ? 'text-[hsl(var(--destructive))]' : 'text-[hsl(var(--warning))]'}>
                                                    {(d.anomalyRate * 100).toFixed(0)}% anomaly
                                                </span>
                                                {d.onTimeRatePct !== null && (
                                                    <span className={d.onTimeRatePct < 60 ? 'text-[hsl(var(--destructive))]' : 'text-[hsl(var(--warning))]'}>
